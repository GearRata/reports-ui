...
interface BranchChartDataPoint {
  ...
  // Dynamic keys like: "สำนักงานใหญ่_pending", "สำนักงานใหญ่_solved"
}
...
export function BranchAreaChart({
    ...
    try {
      ...
      const chartPoints: BranchChartDataPoint[] = allDates.map((dateStr) => {
        ...
        branches.forEach((branch) => {
          ...
          dataPoint[`${branch.name}_pending`] = branchData.pending;
          dataPoint[`${branch.name}_solved`] = branchData.solved;
        });
   ...
   *
   * This function:
   * 1. Creates config for each branch's pending/solved lines
  ...
  const chartConfig = React.useMemo<ChartConfig>(() => {
    ...
    availableBranches.forEach((branch, index) => {
      ...
      // Pending line configuration
      config[`${branch.name}_pending`] = {
        label: `${branch.name} (รอดำเนินการ)`,
        color: baseColor,
      };

      // Solved line configuration (same color but with opacity)
      config[`${branch.name}_solved`] = {
        label: `${branch.name} (แก้ไขแล้ว)`,
        color: baseColor,
      };
    });
      ...
      <Card>
        <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
          <div className="grid flex-1 gap-1 text-center sm:text-left">
            ...
            <CardDescription>
              แสดงจำนวนปัญหาที่รอดำเนินการและแก้ไขแล้วของแต่ละสาขาในแต่ละวัน
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent className="flex items-center justify-center h-[400px]">
        ...
        {!selectedMonth ? (
          ...
          >
            <AreaChart data={chartData}>
                  ...
                  return (
                    <div className="rounded-lg border bg-background p-3 shadow-md">
                      ...
                      <div className="space-y-1">
                        {availableBranches.map((branch, index) => {
                          const pendingCount =
                            (dataPoint[`${branch.name}_pending`] as number) ||
                            0;
                          const solvedCount =
                            (dataPoint[`${branch.name}_solved`] as number) || 0;
                          const total = pendingCount + solvedCount;
              ...
              {availableBranches.map((branch, index) => (
                <React.Fragment key={branch.id}>
                  {/* Solved area (background) */}
                  <Area
                    dataKey={`${branch.name}_solved`}
                    type="natural"
                    fill={`url(#fill${branch.name}Solved)`}
                    stroke={chartConfig[`${branch.name}_solved`]?.color}
                    strokeWidth={1}
                    strokeOpacity={0.6}
                    stackId={`branch-${index}`}
                  />
                  {/* Pending area (foreground) */}
                  <Area
                    dataKey={`${branch.name}_pending`}
                    type="natural"
                    fill={`url(#fill${branch.name}Pending)`}
                    stroke={chartConfig[`${branch.name}_pending`]?.color}
                    strokeWidth={2}
                    stackId={`branch-${index}`}
                  />
                </React.Fragment>

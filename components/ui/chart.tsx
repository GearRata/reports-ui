...
const ChartTooltipContent: React.FC<ChartTooltipContentProps> = (props) => {
  ...
  const safePayload: RechartsPayloadItem[] = React.useMemo(() => Array.isArray(payload) ? payload : [], [payload]);
    ...
    >
      ...
      <div className="grid gap-1.5">
        {safePayload.map((item, index) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          const indicatorColor = color || (item.payload && typeof item.payload === "object" ? (item.payload as { fill?: string }).fill : undefined) || item.color;
          return (
            <div
